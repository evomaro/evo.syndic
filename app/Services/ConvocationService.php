...
class ConvocationService
{
    ...
    public function issue(Assembly $assembly, User $actor, bool $lateException = false, ?string $reason = null): Convocation
    {
        return DB::transaction(function () use ($assembly, $actor, $lateException, $reason) {
            $assembly = Assembly::query()->whereKey($assembly->id)->with(['residence', 'mandate', 'activeAgendaVersion', 'agendaItems.resolution.ruleSnapshot', 'electorate'])->lockForUpdate()->firstOrFail();
            ...
            $convocation = Convocation::create(['organization_id' => $assembly->organization_id, 'residence_id' => $assembly->residence_id, 'assembly_id' => $assembly->id, 'issued_at' => now('UTC'), 'issued_by' => $actor->id, 'legal_deadline_at' => $latestOnTime, 'late_exception' => $lateException, 'late_exception_reason' => $reason, 'path' => $path, 'checksum' => hash('sha256', $pdf), 'frozen_payload' => $payload, 'agenda_version_id' => $assembly->active_agenda_version_id, 'legal_service_status' => 'unverified']);
            ...
            $assembly->activeAgendaVersion?->update(['status' => 'issued', 'issued_at' => now('UTC')]);
    ...
    {
        return ['assembly' => ['id' => $assembly->id, 'reference' => $assembly->reference, 'type' => $assembly->type, 'convocation_number' => $assembly->convocation_number, 'convening_authority' => $assembly->convening_authority, 'meeting_date' => $assembly->meeting_date->toDateString(), 'starts_at' => substr($assembly->starts_at, 0, 5), 'location' => $assembly->location, 'timezone' => $assembly->timezone], 'residence' => ['id' => $assembly->residence_id, 'name' => $assembly->residence->name, 'address' => $assembly->residence->address_line_1, 'city' => $assembly->residence->city], 'agenda_version' => $assembly->activeAgendaVersion?->only(['id', 'version', 'checksum']), 'agenda' => $assembly->agendaItems->where('status', 'frozen')->sortBy('display_order')->map(fn ($i) => ['id' => $i->id, 'version' => $i->version, 'order' => $i->display_order, 'title_fr' => $i->title_fr, 'title_ar' => $i->title_ar, 'proposed_text_fr' => $i->resolution?->proposed_text_fr, 'proposed_text_ar' => $i->resolution?->proposed_text_ar, 'rule_checksum' => $i->resolution?->ruleSnapshot?->checksum])->values()->all(), 'documents' => $assembly->documents->where('status', 'published')->map(fn ($d) => ['id' => $d->id, 'version_id' => $d->published_version_id, 'checksum' => $d->publishedVersion?->checksum])->values()->all(), 'legal' => ['classification' => 'unverified_technical_preview', 'notice' => 'Not certified and not legal advice. Delivery evidence does not prove legally valid service.', 'legacy_source' => config('governance.legal_source.official_source'), 'review_status' => config('governance.legal_source.review_status')]];
    }
}
