...
{
    public function evaluate(?CarbonImmutable $date = null, bool $apply = true): array
    {
        $date ??= CarbonImmutable::today();
        $counts = ['upcoming' => 0, 'execution_due' => 0, 'execution_overdue' => 0, 'mandate_expiring' => 0];
        Assembly::with('electorate')->whereIn('status', ['convocation_issued', 'scheduled'])->whereBetween('meeting_date', [$date->toDateString(), $date->addDays(3)->toDateString()])->each(function ($a) use (&$counts, $apply) {
        ...
        ResolutionExecutionAction::with('resolution.assembly')->whereIn('status', ['pending', 'in_progress'])->whereNotNull('responsible_user_id')->whereDate('due_on', '<=', $date->addDays(3))->each(function ($x) use (&$counts, $date, $apply) {
        ...
        GovernanceMandate::with('residence')->where('status', 'active')->whereBetween('ends_on', [$date, $date->addDays(30)])->each(function ($m) use (&$counts, $apply, $date) {
